...
libs="-lcolors -lconfig -lcurrency -ldefine -ldice -lfunctions -lscope -lvar"
...
if [[ $EUID -eq 0 ]]; then
	...
	g++ $flags src/banner.cpp -L $lib_dir $libs $debug -o /bin/rpgsh-banner
	echo -e "\tsrc/clear.cpp\t\t->\t$bold_white""/bin/rpgsh-clear"$normal
	g++ $flags src/clear.cpp -L $lib_dir $libs $debug -o /bin/rpgsh-clear
	...
	g++ $flags src/history.cpp -L $lib_dir $libs $debug -o /bin/rpgsh-history
	echo -e "\tsrc/list.cpp\t\t->\t$bold_white""/bin/rpgsh-list"$normal
	g++ $flags src/list.cpp -L $lib_dir $libs $debug -o /bin/rpgsh-list
	echo -e "\tsrc/load.cpp\t\t->\t$bold_white""/bin/rpgsh-load"$normal
	g++ $flags src/load.cpp -L $lib_dir $libs $debug -o /bin/rpgsh-load
	echo -e "\tsrc/new.cpp\t\t->\t$bold_white""/bin/rpgsh-new"$normal
	g++ $flags src/new.cpp -L $lib_dir $libs $debug -o /bin/rpgsh-new
	echo -e "\tsrc/print.cpp\t\t->\t$bold_white""/bin/rpgsh-print"$normal
	g++ $flags src/print.cpp -L $lib_dir $libs $debug -o /bin/rpgsh-print
	echo -e "\tsrc/roll.cpp\t\t->\t$bold_white""/bin/rpgsh-roll"$normal
	g++ $flags src/roll.cpp -L $lib_dir $libs $debug -o /bin/rpgsh-roll
	echo -e "\tsrc/setname.cpp\t\t->\t$bold_white""/bin/rpgsh-setname"$normal
	g++ $flags src/setname.cpp -L $lib_dir $libs $debug -o /bin/rpgsh-setname
	echo -e "\tsrc/valueof.cpp\t\t->\t$bold_white""/bin/rpgsh-valueof"$normal
	g++ $flags src/valueof.cpp -L $lib_dir $libs $debug -o /bin/rpgsh-valueof
	echo -e "\tsrc/version.cpp\t\t->\t$bold_white""/bin/rpgsh-version"$normal
	g++ $flags src/version.cpp -L $lib_dir $libs $debug -o /bin/rpgsh-version
