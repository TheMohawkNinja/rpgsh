...
std::string doAction(VariableInfo* p_vi, Action action, std::string value)
{
	...
	if(p_vi->variable.back() != '/')// If the last character isn't a '/', just handle v
	{
		...
		if(p_vi->evalType == VAR_SIGIL)
		{
			...
			else if(action == a_read && keyExists && !stringcasecmp(p_vi->property,"Value"))
		...
		else if(p_vi->evalType == DICE_SIGIL)
		{
			...
			else if(action == a_read && keyExists &&
			...
			else if(action == a_write && !stringcasecmp(p_vi->property,"Quantity"))
				p_vi->scope.setProperty<Dice,int>(p_vi->key,p_vi->property,std::stoi(v));
			else if(action == a_write && !stringcasecmp(p_vi->property,"Faces"))
				p_vi->scope.setProperty<Dice,int>(p_vi->key,p_vi->property,std::stoi(v));
			else if(action == a_write && !stringcasecmp(p_vi->property,"Modifier"))
				p_vi->scope.setProperty<Dice,int>(p_vi->key,p_vi->property,std::stoi(v));
		...
		else if(p_vi->evalType == CURRENCY_SIGIL)
		{
			...
			else if(action == a_write && !stringcasecmp(p_vi->property,"CurrencySystem"))
				p_vi->scope.setProperty<Currency,std::string>(p_vi->key,p_vi->property,v);
			else if(action == a_write && !stringcasecmp(p_vi->property,"Name"))
				p_vi->scope.setProperty<Currency,std::string>(p_vi->key,p_vi->property,v);
			else if(action == a_write && !stringcasecmp(p_vi->property,"Smaller"))
				p_vi->scope.setProperty<Currency,std::string>(p_vi->key,p_vi->property,v);
			else if(action == a_write && !stringcasecmp(p_vi->property,"Larger"))
				p_vi->scope.setProperty<Currency,std::string>(p_vi->key,p_vi->property,v);
	...
	else if(action == a_set_add || action == a_set_add_a ||
		action == a_remove_add || action == a_remove_add_a)// Perform a_write operation on variable set
	{
		struct RemovedKey
		{
		...
		if(action == a_remove_add || action == a_remove_add_a)
		{
...
void parseLHSAndDoOp(VariableInfo* vi, std::vector<std::string>* v, unsigned int lhs_pos, unsigned int op_pos, unsigned int rhs_pos)
{
	...
	if(lhs_pos == 0 && vi->key != "" &&
	  (findInVect<std::string>(assignOps,(*v)[op_pos]) > -1 || findInVect<std::string>(unaryOps,(*v)[op_pos]) > -1))
		(void)doAction(vi, a_write, result);
...
int main(int argc, char** argv)
{
	...
	if(argc == 2)// Print data if all the user enters is a variable
	{
		...
		std::string str = doAction(&vi, a_read, "");
	...
	else// Perform operation on set
	{
		...
		if(!strcasecmp(argv[2],OP_ADD))
			...
			(void)doAction(&vi, a_set_add_a, rhs);
		else if(!strcasecmp(argv[2],OP_SUB))
			fprintf(stdout,"%s\n",doAction(&vi, a_remove_add, rhs).c_str());
		else if(!strcasecmp(argv[2],OP_SUBA))
			(void)doAction(&vi, a_remove_add_a, rhs);
	}
	return 0;
}
